...
void subsample(RGB rgb[8][8], short cb[8][8], short cr[8][8])
{
	RGB pixel;
...
int main (int argc, char *argv[])
{
	...
	for (unsigned y = 0; y < IMG_HEIGHT; y += 8) {
		for (unsigned x = 0; x < IMG_WIDTH; x += 8)
		{
			if (!rgb_get_block(x, y, 8, 8, (RGB*)RGB8x8,
				mmap_addr)) {
			...
			for (unsigned c = 0; c < 8; c++)
			{
				const color R = RGB8x8[r][c].Red;
				const color G = RGB8x8[r][c].Green;
				const color B = RGB8x8[r][c].Blue;
				// converting RGB into Y (luminance)
				Y8x8[r][c] = RGB2Y(R, G, B)-128;
			}
			
			//getting Cb and Cr
			subsample(RGB8x8, Cb8x8, Cr8x8);
